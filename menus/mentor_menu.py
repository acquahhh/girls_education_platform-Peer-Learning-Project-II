...
def ui_register_mentor():
    ...
    if mentor_service.register_mentor(fullname, profession, email, phone):
        print("[OK] Mentor profile created successfully inside the database!")
    pause()
...
def ui_update_mentor():
    ...
    if mentor_service.update_mentor(
        exact_match.mentor_id, fullname, profession, email, phone
    ):
        print("[OK] Mentor profile updated successfully!")
    else:
...
def ui_delete_mentor():
    ...
    if confirm == "yes":
        if mentor_service.delete_mentor(mentor_id):
            print("[OK] Mentor record wiped from the database successfully.")
        else:
